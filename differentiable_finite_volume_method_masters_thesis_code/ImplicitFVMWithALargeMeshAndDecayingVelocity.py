...
def create_value_grid_with_decay(x_size, y_size, central_coordinates, central_coordinates_value, decay_rate):
    """
    Create a grid of dimensions x_size by y_size, setting values influenced by the distance from a central point.

    Parameters:
    - x_size (int): Number of columns in the grid.
    - y_size (int): Number of rows in the grid.
    - central_coordinates (tuple): The central coordinate (x, y) from which the value decays.
    - central_coordinates_value (float): Initial value at the central coordinate.
    - decay_rate (float): Rate at which the value decays with distance.

    Returns:
    - np.ndarray: The resulting grid with values set based on distance decay.
    """
    # Create the initial grid of zeros
    grid = np.zeros((y_size, x_size), dtype=float)

    # Calculate the decayed value based on the Euclidean distance from the central coordinate
    for y in range(y_size):
        for x in range(x_size):
            distance = np.sqrt((central_coordinates[0] - x) ** 2 + (central_coordinates[1] - y) ** 2)
            decayed_value = central_coordinates_value - decay_rate * distance
            grid[y, x] = max(decayed_value, 0)  # Ensuring that no negative values are assigned
...
x_size = 50
y_size = 50
central_coordinates = (25, 25)  # Central point in a 10x10 grid
central_coordinates_value = 10  # Initial value at the central point
decay_rate = 0.35               # Value decreases by 1 per unit distance
values_array = create_value_grid_with_decay(x_size, y_size, central_coordinates, central_coordinates_value, decay_rate)
print(values_array)

values_tensor = tensor(values_array, spatial('y', 'x'))
values_tensor = pack_dims(values_tensor, spatial, instance(mesh_test))
values_vector = vec(x=values_tensor, y=values_tensor * .35)
